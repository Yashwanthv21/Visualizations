...
class _BarChartRace:
    ...
    def validate_params(self):
        ...
        if self.bar_label_position not in ("outside", "inside", None):
            raise ValueError(
                '`bar_label_position` must be one of "outside", "inside" or None'
            )
    ...
    def get_bar_colors(self, cmap):
        ...
        if isinstance(cmap, str):
            ...
            try:
                ...
                raise KeyError(
                    f"Colormap {cmap} does not exist. Here are the "
            ...
            raise TypeError(
                "`cmap` must be a string name of a colormap, a matplotlib colormap "
    ...
    def get_subplots_adjust(self):
        ...
        bars = plot_func(bar_location, bar_length, tick_label=cols)

        self.prepare_axes(ax)
        texts = self.add_bar_labels(ax, bar_location, bar_length, bars)
    ...
    def plot_bars(self, ax, i):
        ...
        self.add_bar_labels(ax, bar_location, bar_length, bars)
    ...
    def add_bar_labels(self, ax, bar_location, bar_length, bars=None):
        ...
        if self.bar_label_position:
            ...
            text_objs = []
            for idx, (x1, y1) in enumerate(zipped):
                ...
                im = ax.imshow(arr_logo, extent=(x1, x1 + 2, y0, y1))
                ax.add_image(im)
